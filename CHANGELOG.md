...
- **Entity name resolution for deletes** -- `resolveObjectId()` resolves opaque IDs (numeric, UUID, or composite) to human-readable names for GET delete and AJAX delete events. Session-based entity cache (`ENTITY_CACHE_MAP`) covers 23 modules: Userman, IVR, Time Conditions, Announcements, Trunks, Contact Manager, Certman, Backup, Calendar, Custom Destinations, Custom Extensions, Ring Groups, Queues, Conferences, Paging, Misc Destinations, Day/Night (Call Flow Control), PIN Sets, Languages, Music on Hold, Call Recording, DISA, Outbound Routes, and Inbound Routes (with composite key support). Cache is populated on every non-state-changing GET page view and after create events.
...
- **Expanded object ID detection** -- `detectObjectId()` now recognizes `pagenbr`, `announcement_id`, `callrecording_id`, `channel`, `orig_account`, `trunknum`, `destid`, `custom_exten`, `old_custom_exten`, `language_id`, `page_group`, `route_id`, `disa_id`, and additional module-specific ID fields.
