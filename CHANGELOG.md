...
- **Richer AJAX change payloads** -- `buildAjaxChangePayload()` now captures `scheme`, `scheme_name`, `source` (Superfecta), `state` (Followme toggleFM), `blockType` (Blacklist), `destination` (Hotelwakeup, Queueprio), `categoryid` (Music), `priority_name` (Queueprio), `fileid`/`filepath` (Backup restore).
- **JS interceptor body-snippet expansion** -- `idKeys` array expanded with `scheme_name`, `source`, `priority_name`, `state`, `blockType`, `destination`, `categoryid`, `fileid`, `filepath`, `certaction`, `submittype`, `day`, `time`, `key`, `value`, `replace` for comprehensive parameter capture across all module patterns.
...
- **JS interceptor body-snippet enrichment** -- The `extractSnippet()` helper now captures `command`, `action`, `svc` (service), `zone`, `protocol`, `setting`, `val`, `namefile`, `data`, `log_file`, `network`, `host`, `ip`, `fw_popover_process`, and `dlg_mode` parameters from request bodies, improving audit detail for firewall, logfiles, network management, and dialog-based operations.
