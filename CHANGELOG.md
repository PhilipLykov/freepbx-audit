...
- **Entity name resolution for all actions** -- `resolveObjectId()` resolves opaque IDs (numeric, UUID, or composite) to human-readable names for ALL event types (create, edit, delete, sensitive reads), not just create/add actions. Session-based entity cache (`ENTITY_CACHE_MAP`) covers 27 modules: Userman, IVR, Time Conditions, Announcements, Trunks, Contact Manager, Certman, Backup, Calendar, Calendar Groups, Custom Destinations, Custom Extensions, Ring Groups, Queues, Conferences, Paging, Parking, Misc Destinations, Misc Applications, Day/Night (Call Flow Control), PIN Sets, Languages, Music on Hold, Call Recording, DISA, Outbound Routes, and Inbound Routes (with composite key support). Cache is populated on every non-state-changing GET page view and after create events.
...
- **Misc Applications entity cache** -- `Miscapps::listApps()` added to `ENTITY_CACHE_MAP` (now 27 modules) with `miscapps_id` in all object ID candidate lists.
