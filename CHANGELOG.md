...
- **Entity name resolution for all actions** -- `resolveObjectId()` resolves opaque IDs (numeric, UUID, or composite) to human-readable names for ALL event types (create, edit, delete, sensitive reads), not just create/add actions. Session-based entity cache (`ENTITY_CACHE_MAP`) covers 26 modules: Userman, IVR, Time Conditions, Announcements, Trunks, Contact Manager, Certman, Backup, Calendar, Calendar Groups, Custom Destinations, Custom Extensions, Ring Groups, Queues, Conferences, Paging, Parking, Misc Destinations, Day/Night (Call Flow Control), PIN Sets, Languages, Music on Hold, Call Recording, DISA, Outbound Routes, and Inbound Routes (with composite key support). Cache is populated on every non-state-changing GET page view and after create events.
...
- **Expanded object ID detection** -- `detectObjectId()` now recognizes `pagenbr`, `announcement_id`, `callrecording_id`, `channel`, `orig_account`, `trunknum`, `destid`, `custom_exten`, `old_custom_exten`, `language_id`, `page_group`, `route_id`, `disa_id`, `number`, `oldval`, and additional module-specific ID fields.
- **Misc Destinations BMO hooks** -- `Miscdests::add()`, `Miscdests::del()`, `Miscdests::update()` are now intercepted via BMO method hooks using `captureEarlyExitEvent()`, which bypasses the timing issue where modules call `redirect_standard()` → `exit()` before the audit GUI hook fires.
